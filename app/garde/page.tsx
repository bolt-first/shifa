...
async function fetchPharmacies(which: string) {
  const res = await fetch(`http://localhost:8003/all-pharmacies?which=${which}`);
  if (!res.ok) {
...
export default function GardePage() {
  ...
  useEffect(() => {
    const loadPharmacies = async () => {
      try {
        setLoading(true);
        const data = await fetchPharmacies('garde');
        setPharmacies(data);
      } catch (err) {
  ...
  const sortedPharmacies = React.useMemo(() => {
    ...
    if (isActive && latitude && longitude) {
      result = result.map(pharmacy => ({
        ...pharmacy,
        distance: `${calculateDistance(latitude ?? 0, longitude ?? 0, pharmacy.latitude ?? 0, pharmacy.longitude ?? 0).toFixed(1)} km`
      })).sort((a, b) => {
